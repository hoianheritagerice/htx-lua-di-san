...
function veBanDo(){
  ...
  d.plots.forEach((p,i)=>{
    ...
    s += `<polygon id="thua${i}" class="thua chua-ro" points="${pts}" stroke-width="${netVien}" data-sw="${netVien}" onclick="chonThua(${i})"/>`;
  });
  d.plots.forEach(p=>{
...
function apDungKetQua(r){
  ...
  MAP_DATA.plots.forEach((p,i)=>{
    ...
    if(nd){
      khop++;
      el.classList.add(nd.organic ? 'huu-co' : 'khong-huu-co');
      if(nd.organic) hoHuuCo.add(p.farmer.trim().toUpperCase());
    }
    else el.classList.add('chua-ro');
  });
  tt.textContent = `Đã khớp ${khop}/${MAP_DATA.plots.length} thửa (vụ ${$('selVu').value}).`;
...
async function napNotion(){
  const tt = $('trangthai');
  tt.className='';
  const khoaDem = 'htxmap_' + MAP_DATA.field + '_' + $('selVu').value;
