...
const names={frog:'Ếch nhảy giữa bờ và nước',dragonfly:'Chuồn chuồn bay trên mặt nước',fish:'Đàn cá nhỏ trong dòng nước',snail:'Ốc bò trên bờ ẩm',duck:'Vịt bơi giữa bụi lúa',worm:'Trùn ngọ nguậy dưới đất',grasshopper:'Châu chấu nhảy trên bờ',bird:'Chim chuyền cành',nest:'Chim non trong tổ',mouse:'Chuột chạy bên bụi cỏ',snake:'Rắn trườn trên bờ',crab:'Cua bên mép nước'};
...
function placements(layout){return layout.points.map((p,i)=>{
 if(layout.mobile){const w=Math.min(300,layout.width-12);return {x:(layout.width-w)/2,y:Math.max(p.label.y+p.label.height+14,p.y+86),width:w,height:w*.6,type:types[i%types.length]};}
 ...
 return {x:x+(p.label.width-w)/2,y:p.y+3,width:w,height:h,type:types[i%types.length]};
});}
function load(){
 ...
 loading=Promise.all([get('img/sinh-thai/ban-phac-thao-da-duyet.png'),get('img/sinh-thai/ech-chuyen-dong.png')]).then(([approved,frog])=>{
 sprites=window.HeSinhThaiArt.makeSprites(approved,frog,(w,h)=>{const c=document.createElement('canvas');c.width=w;c.height=h;return c;});
...
function paint(s){if(!sprites)return;const g=s.canvas.getContext('2d');g.setTransform(s.canvas.width/320,0,0,s.canvas.height/192,0,0);window.HeSinhThaiArt.draw(g,sprites,s.type,s.time);s.el.classList.add('eco-ready');s.el.dataset.phase=s.type==='frog'?window.HeSinhThaiArt.frogPose(s.time).phase:'habitat';}
...
function render(river,layout){
 ...
 boxes.forEach((b,i)=>{
 let s=scenes[i];if(!s){const el=document.createElement('div');el.className='eco-scene';el.dataset.animal=b.type;el.dataset.name=names[b.type];const canvas=document.createElement('canvas');el.append(canvas);layer.append(el);s={el,canvas,type:b.type,time:0,visible:!observer};scenes.push(s);if(observer)observer.observe(el);else{el.classList.add('eco-entered','eco-visible');load();}}
