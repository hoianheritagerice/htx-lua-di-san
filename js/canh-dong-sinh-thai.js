...
function placements(layout){return layout.points.map((p,i)=>{
 ...
 if(layout.mobile){const w=Math.min(240,layout.width-112);return {x:104,y:Math.max(p.label.y+p.label.height+14,p.y+86),width:w,height:w*.6,...info};}
...
function render(river,layout){
 ...
 boxes.forEach((b,i)=>{
 let s=scenes[i];if(!s){const el=document.createElement('div');el.className='eco-scene';el.dataset.animal=b.type;el.dataset.name=names[b.type];const canvas=document.createElement('canvas');el.append(canvas);layer.append(el);s={el,canvas,type:b.type,stage:b.stage,mobile:layout.mobile,time:0,visible:!observer};scenes.push(s);if(observer)observer.observe(el);else{el.classList.add('eco-entered','eco-visible');load();}}
 if(s.type!==b.type||s.stage!==b.stage){s.type=b.type;s.stage=b.stage;s.time=0;}
 s.mobile=layout.mobile;
