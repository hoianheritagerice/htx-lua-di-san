...
assert.equal(snake(0).tongueOut,false,'Tongue rests inside the mouth');
assert.equal(snake(.5).tongueOut,true,'Tongue briefly flicks out');
assert.equal(snake(.66).tongueOut,false,'Tongue retracts between flicks');
assert.equal(snake(.8).tongueOut,true,'Tongue flicks a second time');
